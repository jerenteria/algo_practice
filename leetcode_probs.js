...
var removeDuplicates = function(nums) {
    // get first pointer to start at index 0
    let left = 0;
    // have second pointer start at index 1 and loop through array
    for(i = 1; i < nums.length; i++) {
        // loops until it the values at the both pointers are not equal to each other stops to check at (if statement)
        // if value on the left does not equal the value at index i
            // l  i
            // [0,0,1,1,1,2,2,3,3,4]
        if(nums[left] !== nums[i]) {
            // then move the left pointer up 1
            //    l i                       l i                     l     i                 l         i               l           i             l         i                      
            // [0,0,1,1,1,2,2,3,3,4] = [0,1,1,1,1,2,2,3,3,4] = [0,1,1,1,1,2,2,3,3,4] = [0,1,2,1,1,1,2,3,3,4] = [0,1,2,3,1,2,2,3,3,4] = [0,1,2,3,4,2,2,3,3,4]
            left++;
            // and set the value on the left to be equal to the value and index i
            nums[left] = nums[i]; // changes the value at index left to the value at index i
            //    l i                       l i                     l     i                 l         i               l           i             l         i                      
            // [0,0,1,1,1,2,2,3,3,4] = [0,1,1,1,1,2,2,3,3,4] = [0,1,1,1,1,2,2,3,3,4] = [0,1,2,1,1,1,2,3,3,4] = [0,1,2,3,1,2,2,3,3,4] = [0,1,2,3,4,2,2,3,3,4]
        }
    }
    // return the index that left stopped at and add 1 because that is the amount of total non duplicates in array
    // [0,1,2,3,4,2,2,3,3,4] here the first 4 indexes are non duplicates but the question asks you to place the non duplicates
    // at the beginning of the array and return how many total non duplicates there are. left has traveled to index 4 but 
    // there are 5 total non duplicates so you add 1 bc no matter the arr it is always going to start at index 0 so you add 1
    // to get the total amount of values that we want
    return left + 1;
};
